...
export default function Exam({ examMode = "certification" }) {
  ...
  useEffect(() => {
    ...
    const handleBlockedShortcuts = (event) => {
      const key = String(event.key || '').toLowerCase();
      if (event.metaKey && key === 'g') {
        event.preventDefault();
        event.stopPropagation();
        setInfoMsg('Windows Game Bar shortcut is blocked during the exam.');
      }
    };

    window.addEventListener('keydown', handleBlockedShortcuts, true);
    return () => {
      window.removeEventListener('keydown', handleBlockedShortcuts, true);
    };
  }, [phase]);
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(() => {
      ...
      const avgBrightness = brightnessTotal / Math.max(1, data.length / 4);
      const isBlankFrame = avgBrightness < 10;
      ...
      if (!blankScreenStartedAtRef.current) {
        blankScreenStartedAtRef.current = Date.now();
        return;
      }

      const blankForSec = (Date.now() - blankScreenStartedAtRef.current) / 1000;
      if (!blankScreenWarnedRef.current && blankForSec >= BLANK_SCREEN_WARNING_SEC) {
        blankScreenWarnedRef.current = true;
        setShowBlankScreenWarning(true);
      }

      if (blankForSec >= BLANK_SCREEN_LOCK_SEC) {
        blankScreenTerminatedRef.current = true;
        setShowBlankScreenWarning(false);
        void terminateExamForViolation("Blank or covered camera screen detected during exam.");
      }
    }, 1500);
                ...
                {phase === EXAM_PHASES.RULES ? (
                  <div className="space-y-5">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                      ...
                      <div className="rounded-lg border border-slate-200 p-3 bg-slate-50">On Mac, fullscreen/camera/mic rules still apply, but the website cannot turn off system features like Siri.</div>
                      <div className="rounded-lg border border-slate-200 p-3 bg-slate-50">Mic permission is required, but silence alone is not treated as a violation.</div>
      ...
      {showBlankScreenWarning && phase === EXAM_PHASES.RUNNING ? (
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
          <div className="max-w-md w-full bg-white rounded-2xl p-6 border border-red-200 shadow-xl space-y-4 text-center">
            ...
            <p className="text-sm text-slate-700">
              Your exam camera appears black, blank, or covered. Fix the camera view now.
            </p>
