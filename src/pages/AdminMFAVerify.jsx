...
export default function AdminMFAVerify() {
  ...
  // Combine code array to string
  const codeStr = code.join("");

  // Auto-verify when 6 digits entered
  useEffect(() => {
    if (codeStr.length === 6 && codeStr.split('').every(d => d)) {
      verify(codeStr);
    }
  ...
  const verify = async (codeValue) => {
    try {
      ...
      if (verifyError) {
        setAlert({ show: true, title: 'Invalid Code', message: 'The MFA code you entered is incorrect.', type: 'error' });
        setCode(["", "", "", "", "", ""]);
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-blue-50 to-blue-200 px-4">
      ...
      <div className="bg-white rounded-2xl shadow-xl p-8 w-full max-w-md flex flex-col items-center gap-6 border-t-8 border-blue-500 animate-fade-in">
        ...
        <button
          ...
          className={`w-full py-3 rounded-lg font-bold text-lg text-white transition-colors ${loading || codeStr.length !== 6 || !codeStr.split('').every(d => d) ? 'bg-blue-300' : 'bg-blue-600 hover:bg-blue-700'}`}
        >
